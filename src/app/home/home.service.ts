...
export class HomeService {
    ...
    onFetchDistrict(id: number) {
        this.Districts = [];
        return this.http.get('https://cdn-api.co-vin.in/api/v2/admin/location/districts/' + id)
            .pipe(
                map((response) => {
                    let districtsArray: District[] = response['districts'];
                    this.Districts.push(...districtsArray);
                    return this.Districts;
                })
            )
    }

    onFetchSlots(distId: string, date: string) {
        this.Info = [];
        return this.http.get('https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/findByDistrict', {
            params: new HttpParams().set('district_id', distId).set('date', date),
        })
            .pipe(
                map((response) => {
                    this.Info.push(...(response['sessions']));
                    return this.Info;
                }))
    }
